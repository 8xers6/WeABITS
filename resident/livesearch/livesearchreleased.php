...
if(!empty($_POST['live_searchrel'])){

$search=$_POST['live_searchrel'];
...
if(strlen($search)>0){
    ...
    $query1 = "SELECT *,lpad(tblrequested_documents.req_no,6,'0') as req_no FROM `tblrequested_documents` LEFT JOIN tbl_residents ON tblrequested_documents.res_id=tbl_residents.res_id  WHERE tbl_residents.res_id=$resid AND tblrequested_documents.status='released' ORDER BY tblrequested_documents.req_no DESC";
