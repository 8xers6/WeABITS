...
if(!empty($_POST['live_searchcancel'])){

$search=$_POST['live_searchcancel'];
...
if(strlen($search)>0){
    ...
    if(mysqli_num_rows($result1)>0){
        ...
        while($row = $result1->fetch_assoc()){
            ...
            <div class="row justify-content-center  ml-0 p-2  mr-0  fw-bold   " style="background-color:gray;  pointer-events: none;">
             ...
             <h3 style="color:gray;"><b>'.$documenttype.'</b></h3>
    ...
    $query1 = "SELECT *,lpad(tblrequested_documents.req_no,6,'0') as req_no FROM `tblrequested_documents` LEFT JOIN tbl_residents ON tblrequested_documents.res_id=tbl_residents.res_id WHERE tbl_residents.res_id=$resid AND tbl_residents.bar_no=$barno AND tblrequested_documents.status='cancelled' ORDER BY tblrequested_documents.req_no DESC";
