...
if(!$uppercase || !$lowercase || !$number || !$specialchars || strlen($new_pass) < 8) {
  ...
  <li> The password must include at least one special character.</li>
    ...
    $_SESSION['success'] = 'danger';
            ...
            $check = "SELECT * FROM tblbarangay WHERE (username='$username' OR `email`='$username') AND `password`='$hashcur'";
            $res = $conn->query($check);

            if($res->num_rows){

                $query 		= "UPDATE tblbarangay SET `password`='$hashnew' WHERE (username='$username' OR email='$username')";	
