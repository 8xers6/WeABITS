<?php 
	...
	if(!isset($_SESSION['username'])){
		if (isset($_SERVER["HTTP_REFERER"])) {
          ...
          if(!empty($_POST['hno']) &&!empty($_POST['resid'])  &&!empty($_POST['lname']) &&!empty($_POST['email'])){
            ...
            if(!empty($resid)){
			   ...
			   $query="UPDATE `tbl_residents` SET   
				...
				if($conn->query($query) === true){
					...
					$query1="UPDATE `tbl_residents` SET   
					...
					$mail->Subject="Email Verification by WeABITS";
					    ...
					    $_SESSION['message'] = 'Resident has been Verified!';
						$_SESSION['success'] = 'success';
						
					
     
       $sql3="UPDATE `tblregistration` SET `status`='completed' WHERE `email`='$email'";
