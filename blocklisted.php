...
<head>
	...
	<title>Blocklisted -  Barangay Management System</title>
</head>
<body>
				...
				<div class="panel-header bg-primary-gradient">
					<div class="page-inner">
						<div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
							<div>
								<h2 class="text-white fw-bold">Blotter</h2>
							</div>
						</div>
					</div>
				</div>
                            ...
                            <div class="card">
								<div class="card-header">
									<div class="card-head-row">
										<div class="card-title"> Residents With Record</div>
                                        <?php if(isset($_SESSION['username'])):?>
										<div class="card-tools">
                                     
											<!----
                                            <a href="model/export_residents_csv.php" class="btn btn-success btn-border btn-round btn-sm" onclick="return confirm('Are you sure you want to Download this File?');">
											...
											<a href="#addblocklisted" data-toggle="modal" class="btn btn-info btn-border btn-round btn-sm">
...
<?php else: ?>
<img src="assets/img/person.png" alt="..." class="img-fluid rounded-circle  " width="30" > <?= ucwords($row['lastname'].', '.$row['firstname'].' '.$row['middlename']) ?>
<?php endif ?> 
														...
														<td>
															<div class="form-button-action">
                                                                ...
                                                                <a type="button" data-toggle="tooltip" href="model/remove_blocklisted.php?resid=<?= $row['res_id'] ?>" onclick="return confirm('Are you sure you want to remove residents from blocklist?');" class="btn btn-link btn-danger" data-original-title="Remove from Blocklist">
   ...
   <!-- Modal -->
               <div class="modal fade" id="addblocklisted" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Add Blocklisted</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form method="POST" action="model/addblocklisted.php" >
                                ...
                                <label>Choose Residents</label>
                                  ...
                                  <?php
                                      $squery = mysqli_query($conn,"SELECT * from tbl_residents  LEFT JOIN tblhousehold on tblhousehold.h_no=tbl_residents.h_no LEFT JOIN tblstreet on tblhousehold.st_id=tblstreet.st_id WHERE tbl_residents.bar_no=$barno  AND tbl_residents.verify_status='verified' AND tbl_residents.blocklisted='No Record' AND tbl_residents.alive=1 ");
                                      while ($row = mysqli_fetch_array($squery)){
                                          echo '
                                              <option value="'.$row['res_id'].'">Resident ID:'.$row['res_id'].' | '.$row['firstname'].'  '.$row['middlename'].'  '.$row['lastname'].' '.$row['suffix'].' |Address: '.$row['household_no'].' '.$row['streetname'].' </option>    
                                          ';
                                      }
                                  ?>
                                              </select>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-primary">Add as Blocklisted</button>
                        </div>
                        </form>
                    </div>
                </div>
